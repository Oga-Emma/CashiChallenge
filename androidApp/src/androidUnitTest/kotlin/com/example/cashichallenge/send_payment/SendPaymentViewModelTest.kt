...
class SendPaymentViewModelTest {
    ...
    fun `when form is invalid, should update state and send error event`() = runTest {
        // Given
        val invalidDto = SendPaymentDto("", 0.0, Currency.USD)
        ...
        viewModel.events.test {
            assertEquals(
                SendPaymentEvent.ShowError("Please fix the validation errors"),
                awaitItem()
            )
        }
    }
    ...
    fun `when payment is successful, should update state and send success event`() = runTest {
        // Given
        val validDto = SendPaymentDto("test@test.com", 100.0, Currency.USD)
